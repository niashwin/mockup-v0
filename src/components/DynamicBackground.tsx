...
const GRADIENTS: Record<TimeOfDay, {
  primary: string;
  secondary: string;
  accent: string;
  description: string;
...
// Dark mode gradients
const DARK_GRADIENTS: Record<TimeOfDay, {
  primary: string;
  secondary: string;
  accent: string;
}> = {
...
export const DynamicBackground: React.FC<DynamicBackgroundProps> = ({
  ...
  const [timeOfDay, setTimeOfDay] = useState<TimeOfDay>(override || getTimeOfDay());
  ...
  return (
    <div className={`relative flex flex-col ${className}`}>
      ...
      <div className={`absolute inset-0 bg-gradient-to-tr ${gradient.secondary} ${darkGradient.secondary}`} />
