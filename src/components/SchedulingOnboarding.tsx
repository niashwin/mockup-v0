...
export const SchedulingOnboarding: React.FC<SchedulingOnboardingProps> = ({
  ...
  const [localPrefs, setLocalPrefs] =
    useState<SchedulingPreferences>(preferences);
  ...
  const updateSlotsAndPrefs = useCallback(
    (newSlots: Set<string>) => {
  ...
  const getMealPeriod = (slotIndex: number): string | null => {
    ...
    if (hour >= MEAL_TIMES.breakfast.start && hour < MEAL_TIMES.breakfast.end)
      return "breakfast";
    if (hour >= MEAL_TIMES.lunch.start && hour < MEAL_TIMES.lunch.end)
      return "lunch";
    if (hour >= MEAL_TIMES.dinner.start && hour < MEAL_TIMES.dinner.end)
      return "dinner";
    return null;
  };
          ...
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-2">
                When do you want to take meetings? Drag to select available
                times.
              </p>
            </div>
            ...
            <div>
              ...
              <div
                ref={gridRef}
                className="border border-border rounded-[7px] overflow-hidden select-none"
                onMouseUp={handleMouseUp}
                onMouseLeave={handleMouseUp}
              >
                {/* Header row with day names */}
                <div className="grid grid-cols-8 bg-background dark:bg-neutral-900 border-b border-border">
                  <div className="p-2 text-[10px] font-medium text-muted-foreground text-center"></div>
                  {DAYS.map(({ key, short }) => (
                    <div
                      key={key}
                    ...
                    (slot, rowIndex) => {
                      ...
                      return (
                        ...
                        >
                          {/* Time label */}
                          <div
                            className={`p-1.5 text-[10px] text-muted-foreground text-right pr-2 flex items-center justify-end ${
                              mealPeriod
                          ...
                          >
                            ...
                            {mealPeriod &&
                              slotIndex ===
                                (MEAL_TIMES[
                                  mealPeriod as keyof typeof MEAL_TIMES
                                ].start -
                                  6) *
                                  2 && (
                                <span className="ml-1 text-[8px] text-yellow-500 font-medium">
                          ...
                          {DAYS.map(({ key }) => {
                            const slot1Selected = selectedSlots.has(
                              `${key}-${slotIndex}`,
                            );
                            const slot2Selected = selectedSlots.has(
                              `${key}-${slotIndex + 1}`,
                            );
                            const bothSelected = slot1Selected && slot2Selected;
                            const partialSelected =
                              slot1Selected || slot2Selected;
                            ...
                            return (
                              <div key={key} className="flex">
                                ...
                                <div
                                  onMouseDown={() =>
                                    handleCellMouseDown(key, slotIndex)
                                  }
                                  onMouseEnter={() =>
                                    handleCellMouseEnter(key, slotIndex)
                                  }
                                  className={`flex-1 h-6 border-r border-border/50 cursor-pointer transition-colors ${
                                    slot1Selected
                                      ? "bg-accent"
                                      : mealPeriod
                                        ? "bg-yellow-50/30 dark:bg-yellow-900/5 hover:bg-accent/20"
                                        : "hover:bg-accent/15"
                                  }`}
                                />
                                {/* Second 30 min */}
                                <div
                                  onMouseDown={() =>
                                    handleCellMouseDown(key, slotIndex + 1)
                                  }
                                  onMouseEnter={() =>
                                    handleCellMouseEnter(key, slotIndex + 1)
                                  }
                                  className={`flex-1 h-6 border-r border-border/50 cursor-pointer transition-colors ${
                                    slot2Selected
                                      ? "bg-accent"
              ...
              <p className="text-[10px] text-muted-foreground mt-2 text-center">
                Click and drag to select time blocks • Shaded areas indicate
                meal meeting times
              </p>
            </div>
          </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Help Sentra understand your work style for better location
                suggestions.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                },
                {
                  ...
                  label: "Hybrid",
                  icon: Home,
                  desc: "Mix of home & office",
                },
                {
                  value: "office",
                  label: "Office",
                  icon: Building2,
                  desc: "Primarily in office",
                },
              ].map((option) => (
                <button
                  key={option.value}
                  onClick={() =>
                    updateLocalPrefs({
                      workLocation: {
                        ...localPrefs.workLocation,
                        mode: option.value as LocationMode,
                      },
                    })
                  }
              ...
              {[
                {
                  ...
                  label: "Breakfast meetings",
                  time: "Before 9 AM",
                },
                {
                  key: "allowLunchMeetings",
                  label: "Lunch meetings",
                  time: "12 - 1 PM",
                },
                {
                  key: "allowDinnerMeetings",
                  label: "Dinner meetings",
                  time: "After 6 PM",
                },
              ].map((item) => (
                <div
                  key={item.key}
                  className="flex items-center justify-between p-3 rounded-[7px] border border-border bg-card dark:bg-neutral-900"
                >
                  <div>
                  ...
                  <Switch
                    checked={
                      localPrefs.meetingContext[
                        item.key as keyof typeof localPrefs.meetingContext
                      ] as boolean
                    }
              ...
              <select
                ...
                onChange={(e) =>
                  updateLocalPrefs({
                    meetingContext: {
                      ...localPrefs.meetingContext,
                      maxMeetingsPerDay: Number(e.target.value),
                    },
                  })
                }
                className="w-full p-2.5 bg-card dark:bg-neutral-900 border border-border rounded-[7px] text-sm"
          ...
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-muted-foreground mt-1 italic">
                Any meetings you have at other places will automatically save
                here.
              </p>
            </div>
            ...
            {localPrefs.savedLocations.length > 0 && (
              <div className="space-y-2">
                {localPrefs.savedLocations.map((loc) => (
                  <div
                    key={loc.id}
                    className="flex items-center gap-3 p-3 rounded-[7px] border border-border bg-card dark:bg-neutral-900"
                  >
            ...
            {/* Add New Location */}
            <div className="p-4 rounded-[7px] border border-dashed border-border bg-background dark:bg-neutral-900/50">
              <div className="space-y-3">
                <input
                  type="text"
                  ...
                  className="w-full p-2 bg-card dark:bg-neutral-800 border border-border rounded-[7px] text-sm"
                />
                <input
                  type="text"
                  ...
                  className="w-full p-2 bg-card dark:bg-neutral-800 border border-border rounded-[7px] text-sm"
                />
                <MapPreview address={newLocationAddress} height="150px" />
                <button
                  onClick={addLocation}
                  disabled={
                    !newLocationName.trim() || !newLocationAddress.trim()
                  }
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-neutral-300 disabled:cursor-not-allowed text-white rounded-[7px] text-sm font-medium transition-colors"
                >
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                Use {"{{name}}"}, {"{{slots}}"}, {"{{signature}}"}, and{" "}
                {"{{company}}"} as placeholders.
              </p>
              <textarea
                ...
                rows={10}
                className="w-full p-3 bg-card dark:bg-neutral-900 border border-border rounded-[7px] text-sm font-mono resize-none"
              />
            </div>

            {/* Scheduling Link Toggle */}
            <div className="flex items-center justify-between p-3 rounded-[7px] border border-border bg-card dark:bg-neutral-900">
              <div>
                <span className="text-sm font-medium text-foreground">
                  Prefer scheduling links
                </span>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Use Calendly, Cal.com, etc.
                </p>
              </div>
              <Switch
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-[7px] border border-border bg-card dark:bg-neutral-900">
                ...
                <p className="text-xs text-muted-foreground">
                  {localPrefs.workingAvailability.weekSchedule
                    .filter((d) => d.enabled)
                    .map(
                      (d) => d.day.charAt(0).toUpperCase() + d.day.slice(1, 3),
                    )
                    .join(", ")}
                  {" • "}
              ...
              <div className="p-4 rounded-[7px] border border-border bg-card dark:bg-neutral-900">
                ...
                <p className="text-xs text-muted-foreground">
                  {localPrefs.workLocation.mode.charAt(0).toUpperCase() +
                    localPrefs.workLocation.mode.slice(1)}
                  {localPrefs.workLocation.officeAddress &&
                    ` • ${localPrefs.workLocation.officeAddress}`}
                </p>
              </div>
              ...
              <div className="p-4 rounded-[7px] border border-border bg-card dark:bg-neutral-900">
                ...
                <p className="text-xs text-muted-foreground">
                  {localPrefs.savedLocations.length} location
                  {localPrefs.savedLocations.length !== 1 ? "s" : ""} saved
                </p>
              </div>
              ...
              <div className="p-4 rounded-[7px] border border-border bg-card dark:bg-neutral-900">
                ...
                <p className="text-xs text-muted-foreground">
                  {localPrefs.communicationStyle.preferSchedulingLinks
                    ? "Scheduling links preferred"
                    : "Email coordination"}
                  {localPrefs.communicationStyle.signature &&
                    ` • Signed as ${localPrefs.communicationStyle.signature}`}
                </p>
              </div>
            </div>
          </div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl bg-card dark:bg-neutral-950 rounded-[2rem] shadow-2xl border border-white/20 dark:border-neutral-800 overflow-hidden flex flex-col max-h-[90vh]"
      >
        {/* Header */}
        <header className="px-8 py-6 border-b border-border flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={step === 0}
              className={`p-2 rounded-full hover:bg-muted dark:hover:bg-neutral-800 transition-colors ${step === 0 ? "opacity-0 pointer-events-none" : "opacity-100"}`}
            >
        ...
        <footer className="px-8 py-6 border-t border-border flex items-center justify-between bg-background/50 dark:bg-neutral-900/50 shrink-0">
          ...
          {step < totalSteps - 1 ? (
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-secondary text-foreground border border-border rounded-[7px] font-medium hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors"
            >
              <span>Continue</span>
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-accent hover:bg-accent/90 text-white rounded-[7px] font-medium transition-colors shadow-lg shadow-accent/20"
            >
