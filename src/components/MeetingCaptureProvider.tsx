...
const MeetingCaptureContext = createContext<
  MeetingCaptureContextType | undefined
>(undefined);
...
function generateMockSnapshot(duration: number): MeetingCaptureSnapshot {
  ...
  return {
    understanding:
      "This discussion focused on the Q2 product launch timeline. The group aligned on a revised schedule and approved additional budget, with marketing impact still to be determined.",
    decisions,
    commitments,
    openQuestions,
    duration,
...
export function MeetingCaptureProvider({
  children,
}: {
  children: React.ReactNode;
}) {
  const [captureState, setCaptureState] = useState<MeetingCaptureState>("idle");
  const [currentCapture, setCurrentCapture] = useState<CapturedMeeting | null>(
    null,
  );
  ...
  const startCapture = useCallback(
    (source: CapturedMeeting["source"] = "manual") => {
      ...
      durationInterval.current = setInterval(() => {
        if (captureStartTime.current) {
          const elapsed = Math.floor(
            (Date.now() - captureStartTime.current.getTime()) / 1000,
          );
  ...
  const stopCapture = useCallback(() => {
    ...
    const duration = captureStartTime.current
      ? Math.floor(
          (endTime.getTime() - captureStartTime.current.getTime()) / 1000,
        )
      : 0;
    ...
    const capturedMeeting: CapturedMeeting = {
      id: `capture-${Date.now()}`,
      startTime:
        captureStartTime.current?.toISOString() || endTime.toISOString(),
  ...
  const saveCapture = useCallback(
    (saveAs: CapturedMeeting["savedAs"], threadId?: string) => {
      if (currentCapture) {
        setCurrentCapture({
          ...currentCapture,
          savedAs: saveAs,
...
export function useMeetingCapture() {
  ...
  if (context === undefined) {
    throw new Error(
      "useMeetingCapture must be used within a MeetingCaptureProvider",
    );
  }
  return context;
}
