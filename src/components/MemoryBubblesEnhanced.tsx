...
export const MemoryBubblesEnhanced: React.FC<MemoryBubblesEnhancedProps> = ({
  onBubbleClick,
}) => {
  ...
  const bubbles: Bubble[] = [
    ...
    },
    {
      ...
      y: 85,
      gradient:
        "from-cyan-500/10 to-blue-500/10 text-cyan-800 dark:text-cyan-200 ring-cyan-500/20",
      icon: User,
      delay: 0.65,
      importance: 6,
      type: "person",
    ...
    },
    {
      ...
      y: 45,
      gradient:
        "from-indigo-500/20 to-purple-500/20 text-indigo-900 dark:text-indigo-100 ring-indigo-500/30",
      icon: Users,
      delay: 0.7,
      importance: 8,
      type: "meeting",
      people: ["Sarah Chen", "Alex Rivera", "Morgan Lee"],
    ...
    },
    {
      ...
      y: 45,
      gradient:
        "from-rose-500/20 to-orange-500/20 text-rose-900 dark:text-rose-100 ring-rose-500/30",
      icon: Calendar,
      delay: 0.75,
      importance: 9,
      type: "meeting",
    ...
    },
    {
      ...
      y: 50,
      gradient:
        "from-teal-500/10 to-emerald-500/10 text-teal-800 dark:text-teal-200 ring-teal-500/20",
      icon: User,
      delay: 0.8,
      importance: 6,
      type: "person",
          ...
          <div className="flex items-center gap-1 bg-white/30 dark:bg-black/30 backdrop-blur-md rounded-full p-1 border border-white/20 shadow-lg pointer-events-auto">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all flex items-center gap-1.5 ${
                viewMode === "bubbles"
                  ? "bg-card dark:bg-neutral-800 text-foreground shadow-md"
                    ...
                    {b.type === "meeting" && b.people.length > 1 ? (
                      ...
                      <div className="flex -space-x-2">
                        {b.people.slice(0, 3).map((person, idx) => (
                          <div
                            key={idx}
                            className="w-8 h-8 rounded-full bg-gradient-to-br from-muted to-neutral-200 dark:from-neutral-700 dark:to-neutral-800 border-2 border-white dark:border-neutral-900 flex items-center justify-center text-[10px] font-bold text-foreground shadow-sm"
                            style={{ zIndex: 3 - idx }}
                          >
                            {getInitials(person)}
                          </div>
                      ...
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-muted to-neutral-200 dark:from-neutral-700 dark:to-neutral-800 border-2 border-white/50 flex items-center justify-center text-sm font-bold text-foreground shadow-md">
          ...
          <div className="space-y-2">
            {sortedBubbles.map((b, index) => (
              <motion.div
                ...
                onClick={() => onBubbleClick?.(b)}
                className="bg-white/40 dark:bg-neutral-800/40 backdrop-blur-md rounded-xl p-4 border border-white/20 dark:border-white/10 hover:bg-white/60 dark:hover:bg-neutral-800/60 cursor-pointer transition-all hover:shadow-lg group"
              >
                <div className="flex items-center gap-4">
                  ...
                  b.people.length > 0 ? (
                      ...
                      {b.type === "meeting" && b.people.length > 1 ? (
                        <div className="flex -space-x-2">
                          {b.people.slice(0, 3).map((person, idx) => (
                            <div
                              key={idx}
                              className="w-10 h-10 rounded-full bg-gradient-to-br from-muted to-neutral-200 dark:from-neutral-700 dark:to-neutral-800 border-2 border-white dark:border-neutral-900 flex items-center justify-center text-xs font-bold text-foreground"
                              style={{ zIndex: 3 - idx }}
                            >
                              {getInitials(person)}
                            </div>
                        ...
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-muted to-neutral-200 dark:from-neutral-700 dark:to-neutral-800 border-2 border-white/50 flex items-center justify-center text-sm font-bold text-foreground">
                          {getInitials(b.people[0])}
                        </div>
                      )}
                    </div>
                    ...
                    <div
                      className={`flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br ${b.gradient} flex items-center justify-center`}
                    >
                      ...
                      className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                        (b.importance || 0) >= 9
                          ...
                          : (b.importance || 0) >= 7
                            ...
                            : "bg-muted dark:bg-neutral-700 text-muted-foreground"
                      }`}
                    >
                      P{b.importance}
                    </div>
