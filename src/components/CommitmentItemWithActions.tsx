    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 text-sm text-muted-foreground py-1">
          <motion.div
            className="w-1.5 h-1.5 rounded-full bg-muted-foreground/40 dark:bg-neutral-600 group-hover/item:bg-accent transition-colors"
            animate={{
      ...
      <AnimatePresence>
        {showSuccessRing && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.6 }}
            className="absolute inset-0 rounded-[7px] border-2 border-accent pointer-events-none z-20"
          />
        )}
      </AnimatePresence>

      <div
        className={`p-3.5 rounded-[7px] border transition-all ${
          isCompleting
            ? "bg-accent/5 border-accent/20"
            : "bg-background/50 dark:bg-neutral-800/30 border-border hover:bg-card dark:hover:bg-neutral-800 hover:border-border dark:hover:border-neutral-700 hover:shadow-md"
        }`}
      >
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            <motion.p
              className={`text-sm font-medium text-foreground mb-1 transition-all ${
                isCompleting ? "line-through opacity-50" : ""
              }`}
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span className="w-1 h-1 rounded-full bg-muted-foreground/40 dark:bg-neutral-600" />
              <span
              ...
              {isHovered && !isCompleting && (
                ...
                >
                  ...
                  <motion.button
                    ...
                    className="w-7 h-7 rounded-[7px] bg-accent/10 hover:bg-accent/20 text-accent flex items-center justify-center transition-colors shadow-sm hover:shadow"
                    title="Complete"
                  >
                  ...
                  <motion.button
                    ...
                    className="w-7 h-7 rounded-[7px] bg-yellow-100 dark:bg-yellow-900/30 hover:bg-yellow-200 dark:hover:bg-yellow-900/50 text-yellow-700 dark:text-yellow-400 flex items-center justify-center transition-colors shadow-sm hover:shadow"
                    title="Snooze"
                  >
                  ...
                  <motion.button
                    ...
                    className="w-7 h-7 rounded-[7px] bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-400 flex items-center justify-center transition-colors shadow-sm hover:shadow"
                    title="Delegate"
                  >
