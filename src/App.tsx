...
/**
 * Generate a mock Contact object from just a person name.
 * Uses deterministic hashing so the same name always produces the same data.
 */
const generateMockContact = (name: string): Contact => {
  ...
  const summaries = [
    ...
    "Influential stakeholder who shapes product direction. Appreciates concise updates.",
    "Trusted collaborator with excellent cross-functional relationships. Prefers brief calls.",
  ];
  const talkingPoints = [
    [
      "Q1 roadmap alignment",
      "Budget approval for new initiative",
      "Team expansion plans",
    ],
    [
      "Partnership renewal timeline",
    ...
    [
      ...
      "Resource allocation for H2",
      "New market opportunities",
    ],
  ];
  const facts = [
    {
      content: "Ran a marathon last year in under 4 hours",
      category: "hobby" as const,
    },
    {
      content: "Has twin daughters starting college this fall",
    ...
    { content: "Enjoys woodworking on weekends", category: "hobby" as const },
    {
      content: "Passionate about climate tech investing",
  ...
  return {
    ...
    insights: {
      ...
      talkingPoints: talkingPoints[absHash % talkingPoints.length],
    ...
    interestingFacts: [
      {
        id: `fact-${absHash}-1`,
        content: facts[absHash % facts.length].content,
        source: "Meeting - Jan 2026",
        extractedAt: daysAgo(14),
        category: facts[absHash % facts.length].category,
      },
    ],
    ...
    interactions: [
      ...
      },
      {
        ...
        summary: "Shared updated proposal deck with revised pricing.",
      },
      {
        id: `int-${absHash}-3`,
        type: "slack",
        date: daysAgo(7),
        subject: "Quick question on specs",
        summary: "Clarified integration requirements for Phase 2.",
      },
    ],
...
const NEUTRAL_MEETING_AVATAR =
  "from-neutral-300 to-neutral-400 dark:from-neutral-600 dark:to-neutral-700";

const getMeetingPersonColor = (_name: string) => NEUTRAL_MEETING_AVATAR;

const getMeetingInitials = (name: string) =>
  name
    .split(" ")
    .map((n) => n[0])
    .join("")
...
const getMeetingTypeIcon = (location: string) => {
  if (location.includes("Zoom") || location.includes("Meet")) {
    return (
    ...
    return (
      <div className="flex items-center gap-1 flex-wrap">
        {attendees.map((name, i) => (
          ...
          >
            <div
              className={`w-4 h-4 rounded-full bg-gradient-to-br ${getMeetingPersonColor(name)} flex items-center justify-center text-[7px] font-bold text-white`}
            >
              {getMeetingInitials(name)}
            </div>
    ...
    <div className="flex items-center">
      <div className="flex -space-x-1.5">
        {visible.map((name, i) => (
          ...
          >
            <div
              className={`w-5 h-5 rounded-full bg-gradient-to-br ${getMeetingPersonColor(name)} flex items-center justify-center text-[7px] font-bold text-white ring-2 ring-background`}
            >
              {getMeetingInitials(name)}
            </div>
          </button>
...
const MeetingsScreen = ({
  ...
  const emailMeeting = emailMeetingId
    ? meetings.find((m) => m.id === emailMeetingId)
    : null;
    ...
    >
      ...
      {selectedDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div
            className="bg-card rounded-lg w-full max-w-3xl shadow-2xl relative animate-in zoom-in-95 duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setSelectedDetailId(null)}
              className="absolute top-4 right-4 p-2 bg-secondary rounded-full hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors z-10"
            >
      ...
      <div className="max-w-5xl mx-auto flex flex-col min-h-full relative">
        <div className="absolute -inset-6 rounded-xl bg-destructive/10 blur-3xl pointer-events-none" />
        <div className="absolute -inset-1 rounded-xl border border-destructive/20 dark:border-destructive/20 pointer-events-none" />
          ...
          <div className="space-y-6 pb-20">
            ...
            <div className="space-y-3 opacity-60 hover:opacity-100 transition-opacity duration-300">
              <div className="flex items-center justify-center pb-2 pt-10">
                <div className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest flex flex-col items-center gap-1 animate-pulse">
                  <ArrowUp size={12} />
                  Scroll for History
                </div>
              </div>
              ...
              {completed.map((meeting) => (
                <div
                  ...
                  className="bg-background dark:bg-neutral-900/50 border border-border rounded-xl p-4 flex items-center justify-between group/card cursor-pointer hover:bg-card transition-colors"
                >
                  <div className="flex items-center gap-4">
                    {getMeetingTypeIcon(meeting.location)}
                    <div>
                      <h3 className="text-sm font-semibold text-foreground group-hover/card:text-foreground">
                        {meeting.title}
                      </h3>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-bold border transition-all shadow-sm cursor-pointer group/btn active:scale-95 ${
                        meeting.isPrivate
                          ? "bg-destructive/10 dark:bg-destructive/15 text-destructive border-destructive/20 dark:border-destructive/20 hover:bg-destructive/15"
                          : "bg-accent/10 dark:bg-accent/20 text-accent border-accent/20 dark:border-accent/30 hover:bg-accent/15"
                      }`}
                    >
                      {meeting.isPrivate ? (
                        <Lock size={10} />
                    ...
                    {meeting.reportStatus === "published" && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1 bg-accent/10 dark:bg-accent/20 text-accent hover:bg-accent/15 dark:hover:bg-accent/25 rounded text-[10px] font-bold border border-accent/20 dark:border-accent/30 transition-all shadow-sm cursor-pointer group/btn active:scale-95"
                      >
                        <MessageSquare
                          size={10}
                    ...
                    {meeting.reportStatus === "published" && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-2 py-1 bg-accent/10 dark:bg-accent/20 text-accent hover:bg-accent/20 dark:hover:bg-accent/30 rounded text-[10px] font-bold border border-accent/20 dark:border-accent/30 transition-all shadow-sm cursor-pointer group/btn active:scale-95"
                      >
                        <Mail
                          size={10}
                          className="group-hover/btn:scale-110 transition-transform"
                    ...
                    {meeting.status === "completed" && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-2 py-1 bg-secondary text-muted-foreground hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded text-[10px] font-bold border border-border transition-all shadow-sm cursor-pointer group/btn active:scale-95"
                      >
                        <Share2 size={10} />
                        Share
                      </button>
                    ...
                    <ChevronRight
                      size={14}
                      className="text-muted-foreground/40 group-hover/card:translate-x-0.5 transition-transform"
                    />
                  </div>
                </div>
              ))}
            ...
            >
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-accent bg-accent/10 dark:bg-accent/20 px-3 py-1 rounded-full border border-accent/20 dark:border-accent/30 shadow-sm">
            ...
            <section className="space-y-4">
              {upcoming.length > 0 && (
                <MeetingDetailCard
                  ...
                  onPersonClick={(name) => setSelectedPersonName(name)}
                />
              )}
              ...
              <div className="space-y-3 pt-4">
                ...
                {upcoming.slice(1).map((meeting) => (
                  <div
                    ...
                    className="bg-card border border-border rounded-xl p-5 shadow-sm opacity-90 hover:opacity-100 transition-opacity cursor-pointer group"
                  >
        ...
        {/* Person Detail — uses the CRM ContactDrawer for consistent formatting */}
        <ContactDrawer
          contact={
