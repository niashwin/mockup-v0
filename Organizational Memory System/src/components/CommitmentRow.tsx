...
export const CommitmentRow = ({
  ...
  const DetailsPanel = () => (
    <AnimatePresence>
      {isExpanded && (
        ...
        >
          <div
            className={`pt-3 mt-3 border-t ${isCard ? "border-zinc-100 dark:border-zinc-800/50" : "border-transparent pl-9"}`}
          >
            {commitment.source ? (
              <div className="flex items-start gap-3 bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-zinc-600 dark:text-zinc-300 font-medium mb-1 truncate">
  ...
  return (
    <div
      className={`relative overflow-hidden border rounded-2xl flex group ${containerClasses} ${cardClasses} ${isExpanded ? "ring-1 ring-zinc-300 dark:ring-zinc-600" : ""}`}
    >
      {/* Progress Overlay */}
      <AnimatePresence>
        {isTransitioning && (
      ...
      {isCard ? (
        ...
        <div className="relative z-10 w-full flex flex-col h-full justify-between">
          <div className="flex justify-between items-start w-full mb-2">
            <span
              className={`text-[10px] px-2 py-1 rounded-md border font-bold uppercase tracking-wider ${commitment.priority === "High" ? "bg-red-50 text-red-600 border-red-100 dark:bg-red-900/20 dark:border-red-900/30 dark:text-red-400" : "bg-zinc-100 text-zinc-500 border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700"}`}
            >
              {commitment.priority}
            </span>
            <div
          ...
          <div className="flex-1 mb-2">
            <h3
              className={`text-lg font-bold leading-tight mb-2 ${isCompleted ? "text-zinc-500 line-through decoration-zinc-300" : "text-zinc-900 dark:text-zinc-100"}`}
            >
              {commitment.title}
            </h3>
          ...
          <div className="relative z-10 flex items-center gap-4 w-full">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                <h3
                  className={`font-medium truncate pr-4 ${isCompleted ? "text-zinc-500 line-through decoration-zinc-300" : "text-zinc-900 dark:text-zinc-100"}`}
                >
                  {commitment.title}
                </h3>
                {/* Right side info */}
                <div className="flex items-center gap-3">
                  ...
                  <div
                    className={`text-xs font-medium px-2 py-1 rounded shrink-0 ${isOverdue && !isCompleted ? "bg-red-50 text-red-600" : isHistory ? "text-zinc-400" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400"}`}
                  >
                    {commitment.dueDate}
                  </div>
                </div>
              ...
              {!isHistory && (
                <div className="flex items-center gap-2 mt-1">
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded border ${commitment.priority === "High" ? "bg-red-50 text-red-600 border-red-100 dark:bg-red-900/20 dark:border-red-900/30 dark:text-red-400" : "bg-zinc-100 text-zinc-500 border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700"}`}
                  >
                    {commitment.priority}
                  </span>
