...
export const CommitmentItemWithActions: React.FC<CommitmentItemWithActionsProps> = ({
  commitment,
  onComplete,
  onSnooze,
  onDelegate,
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400 py-1">
          ...
          />
          <span className={`truncate text-xs font-medium tracking-wide transition-all ${
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 text-xs text-zinc-500">
              <span>To: <span className="font-medium text-zinc-600 dark:text-zinc-400">{commitment.assignee}</span></span>
              <span className="w-1 h-1 rounded-full bg-zinc-300 dark:bg-zinc-600" />
              <span className={`font-medium ${
                commitment.status === 'overdue' ? 'text-red-600 dark:text-red-400' : ''
              }`}>
                {commitment.dueDate}
              </span>
            </div>
