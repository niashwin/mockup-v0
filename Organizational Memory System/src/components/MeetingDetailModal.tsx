    ...
    <AnimatePresence>
      {meeting && (
        <>
          ...
          >
            <div
              className={`w-full max-w-4xl max-h-[85vh] bg-white dark:bg-zinc-900 border shadow-2xl shadow-black/20 rounded-[2rem] overflow-hidden flex flex-col pointer-events-auto relative ${isPrivate ? "border-orange-200/70 dark:border-orange-900/40 ring-1 ring-orange-200/40 dark:ring-orange-900/30" : "border-[color:var(--app-border)]"}`}
            >
              {/* Header */}
              <div
                className={`shrink-0 px-8 py-6 border-b border-zinc-100 dark:border-zinc-800/50 relative z-10 ${isPrivate ? "bg-orange-50/60 dark:bg-orange-900/20" : "bg-white dark:bg-zinc-900"}`}
              >
                {/* Decorative gradient */}
                <div
                  className={`absolute top-0 left-1/2 -translate-x-1/2 w-96 h-32 rounded-full blur-3xl pointer-events-none ${isPrivate ? "bg-gradient-to-r from-orange-400/15 via-amber-400/10 to-rose-400/10" : "bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-fuchsia-500/10"}`}
                />
                    ...
                    <div className="flex items-center gap-2">
                      {onTogglePrivacy && (
                        <button
                          ...
                          title={
                            isPrivate ? "Mark as shared" : "Mark as private"
                          }
                        >
                          {isPrivate ? (
                            <Lock size={12} />
                    ...
                    {meeting.reportStatus === "published" && (
                      <div className="relative">
                        ...
                        >
                          ...
                          <ChevronDown
                            size={12}
                            className={`transition-transform duration-200 ${showReportsDropdown ? "rotate-180" : ""}`}
                          />
                        </button>
                                  ...
                                  >
                                    <div className="flex items-start justify-between">
                                      <div className="flex-1 min-w-0">
                                        <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 truncate group-hover/item:text-blue-500 transition-colors">
                                          {report.title}
                                        </h4>
                                        <div className="flex items-center gap-2 mt-1">
                                          <span className="text-[9px] px-1.5 py-0.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 rounded font-bold uppercase">
                                            {report.category}
                                          </span>
                                      ...
                                      <ArrowRight
                                        size={12}
                                        className="text-zinc-300 group-hover/item:text-blue-500 group-hover/item:translate-x-1 transition-all mt-1"
                                      />
                                    </div>
                                  </button>
                                ))}
                      ...
                      {meeting.attendees.map((person, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <ArrowRight
                              size={12}
                              className="text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300 opacity-0 group-hover:opacity-100 transition-all -ml-1 group-hover:ml-0"
                            />
                          </div>
                        </button>
                      ))}
                  ...
                  <section>
                    ...
                    <div className="space-y-2">
                      {mockKeyDecisions.map((decision, idx) => (
                        <div
                          key={idx}
                          className="flex gap-3 p-3 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-xl"
                        >
