...
import { X, Search, Download, Share2, Play, Pause, FastForward, Rewind, MessageSquare, Clock, Users, Sparkles, Filter } from 'lucide-react';
...
const MOCK_TRANSCRIPT: TranscriptEntry[] = [
  ...
  { id: '2', speaker: 'Sarah Chen', time: '00:45', text: "Sure. We've successfully migrated 80% of our core services to the new cluster. We're seeing about a 30% reduction in latency for the organizational memory queries. The remaining 20% should be done by Friday." },
  ...
  { id: '4', speaker: 'Sarah Chen', time: '01:45', text: "The migration actually addresses that directly. The new edge nodes reduce the round-trip time significantly. We're targeting sub-500ms once the migration is complete.", isActionItem: true },
  ...
  { id: '10', speaker: 'Alex Lewis', time: '04:35', text: "Let's sync with the backend team tomorrow morning on that. We need to ensure PII redaction is active before we go to production." },
...
export const TranscriptModal: React.FC<TranscriptModalProps> = ({ isOpen, onClose, meetingTitle = "Q3 Product Roadmap Sync" }) => {
  ...
  const filteredTranscript = MOCK_TRANSCRIPT.filter(item => {
    ...
    if (activeTab === 'highlights') return matchesSearch && item.text.length > 80; // Simple heuristic
          ...
          >
            ...
            <div className="px-8 py-6 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">{meetingTitle}</h2>
                  <div className="flex items-center gap-3 text-xs text-zinc-500 mt-1">
                    <span className="flex items-center gap-1"><Clock size={12} /> 45:12</span>
                    <span className="w-1 h-1 bg-zinc-300 rounded-full" />
                    <span className="flex items-center gap-1"><Users size={12} /> 3 Participants</span>
                    <span className="w-1 h-1 bg-zinc-300 rounded-full" />
                    <span className="text-emerald-500 font-medium">Recording Saved</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800">
                ...
                <div key={entry.id} className="group relative">
                  <div className="flex items-start gap-6">
                    <div className="w-16 pt-1 shrink-0">
                      <span className="text-xs font-mono text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300 transition-colors">{entry.time}</span>
                    </div>
                    <div className="flex-1">
                      <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-2 flex items-center gap-2">
                        {entry.speaker}
                        {entry.isActionItem && (
                          <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-full text-[10px] uppercase tracking-wider font-bold">Action Item</span>
                        )}
                      </h4>
              ...
              {filteredTranscript.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <h3 className="text-zinc-900 dark:text-zinc-100 font-semibold mb-1">No results found</h3>
