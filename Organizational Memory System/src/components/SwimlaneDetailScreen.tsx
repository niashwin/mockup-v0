...
const EventBubble = ({
  ...
  const getColor = (type: string, criticality?: string) => {
    ...
    switch (type) {
      ...
      default:
        return "bg-zinc-500 text-white border-zinc-500 ring-zinc-500/20";
    }
  };
    ...
    >
      ...
      <AnimatePresence>
        {(zoomLevel > 1.5 || isSelected) && (
          ...
          >
            <div
              className={`bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 shadow-lg rounded-xl p-3 min-w-[200px] max-w-[280px] ${isSelected ? "ring-2 ring-emerald-500/50 border-emerald-500/50" : ""} ${item.criticality === "critical" ? "ring-2 ring-red-500/50 border-red-500" : ""}`}
            >
              ...
              <div className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm mb-1 line-clamp-2">
                {item.title}
              </div>
              {isSelected && (
...
export const SwimlaneDetailScreen = ({
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-zinc-500 mt-0.5">
              ...
              <span>•</span>
              <span className="font-medium bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded text-zinc-600 dark:text-zinc-400">
                Last 90 Days
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 rounded-lg p-1 border border-zinc-200 dark:border-zinc-700">
            <button
              ...
              className="p-1.5 hover:bg-white dark:hover:bg-zinc-700 rounded-md text-zinc-500 transition-colors"
            >
              <ZoomOut size={16} />
            </button>
            <button
              onClick={() => setZoom(1)}
              className="px-2 text-xs font-mono text-zinc-500"
            >
              {Math.round(zoom * 100)}%
            </button>
            <button
      ...
      <div
        className={`shrink-0 border-b border-zinc-200 dark:border-zinc-800 ${getStatusColor(lane.status)}`}
      >
        <motion.div
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex -space-x-1.5">
              <div
                className="w-6 h-6 rounded-full bg-white dark:bg-zinc-800 border-2 border-white dark:border-zinc-900 flex items-center justify-center text-[9px] font-bold shadow-sm relative z-20"
          ...
          {processedData.map((item, index) => {
            ...
            const getBgColor = (type: string, criticality?: string) => {
              ...
              switch (type) {
                ...
                default:
                  return "bg-zinc-500/10 border-zinc-500 hover:bg-zinc-500/20";
              }
            };
            ...
            return (
              ...
              >
                <div
                  className={`bg-white dark:bg-zinc-900 border-2 ${bgColor} rounded-xl p-5 shadow-lg min-w-[200px] max-w-[240px] transition-all ${isSelected ? "ring-2 ring-emerald-500 scale-105" : ""} ${item.criticality === "critical" ? "animate-pulse" : ""}`}
                >
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 p-8 overflow-y-auto">
              <div className="max-w-3xl">
                ...
                <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 mb-4 leading-tight">
                  {selectedEvent.title}
                </h2>
                ...
                <div className="mt-8 flex items-center gap-4">
                  <div className="flex -space-x-2">
                    <div className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 bg-zinc-200 flex items-center justify-center text-xs font-bold text-zinc-500">
                      AL
                    </div>
                    <div className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 bg-zinc-200 flex items-center justify-center text-xs font-bold text-zinc-500">
                      SC
              ...
              <div className="space-y-3">
                {selectedEvent.evidence?.map((ev, i) => (
                  <a
                    key={i}
                    href="#"
                    className="block p-3 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg hover:shadow-md transition-all group"
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <ExternalLink
                        size={10}
                        className="text-zinc-300 group-hover:text-emerald-500 transition-colors"
                      />
                    </div>
                    <p className="text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">
                      "{ev.preview}"
                    </p>
                    <div className="text-[10px] text-zinc-400 truncate hover:underline">
                      {ev.url}
                    </div>
                  </a>
                )) || (
